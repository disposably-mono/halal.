  ...
  return (
    <div className="min-h-screen bg-[#0b1220] font-sans">
      ...
      <main className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:px-6">
        ...
        {/* ── Import forms (hidden when finalized) ── */}
        {!election.votersFinalized && (
        ...
        <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#1a2540]">
          ...
          <div className="flex items-center justify-between border-b border-white/[0.07] px-4 py-3">
            ...
            <span className="text-[10px] text-white/25">
              {election._count.voters} total
            </span>
          </div>

          {/* Column headers */}
          <div className="flex items-center gap-3 border-b border-white/[0.05] bg-white/[0.02] px-4 py-2">
            <span className="w-[100px] shrink-0 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/25">
              Voter Code
            </span>
            <span className="flex-1 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/25">
              Student ID
            </span>
            <span className="w-[60px] shrink-0 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/25">
              Grade
            </span>
            <span className="w-[70px] shrink-0 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/25">
              Section
            </span>
            <span className="w-[60px] shrink-0 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/25">
              Voted
          ...
          {voters.length === 0 ? (
            ...
            <div className="max-h-[420px] overflow-y-auto divide-y divide-white/[0.04]">
              {voters.map((voter) => (
                <div
                  key={voter.id}
                  className="flex items-center gap-3 px-4 py-[7px] text-[11px]"
                >
                  <span className="w-[100px] shrink-0 font-mono text-[10px] text-amber-400/80">
                    {voter.voterCode}
                  </span>
                  ...
                  {!election.votersFinalized ? (
                    <form
                      action={async () => {
                        "use server";
                        const fd = new FormData();
                        fd.set("voterId", voter.id);
                        fd.set("electionId", params.id);
                        await removeVoter(fd);
                      }}
                    >
                      <button
                        type="submit"
