    ...
    <div className="p-6 flex flex-col gap-[18px]">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-[11px]">
          ...
          <span className="bg-white/[0.04] border border-white/[0.07] text-white/40 rounded-[6px] px-[9px] py-[4px]">
      ...
      {totalVoters === 0 && (
        <div className="bg-[#1a2540] border border-white/[0.07] rounded-[12px] flex flex-col items-center gap-3 py-16 text-center">
          <div className="text-[13px] font-medium text-white/50">No voters registered yet</div>
          <div className="text-[11px] text-white/30">Upload voters via the election management page.</div>
        </div>
      )}
                      ...
                      <thead>
                        <tr className="border-b border-white/[0.04]">
                          {["Control No.", "Student ID", "Grade", "Section", "Status"].map((h) => (
                            <th key={h} className="text-left px-4 py-[6px] text-[10px] font-semibold uppercase tracking-[0.06em] text-white/25">
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {el.voters.map((v: typeof voters[0]) => (
                          <tr key={v.id} className="border-b border-white/[0.03] last:border-0 hover:bg-white/[0.02] transition-colors">
                            ...
                            <td className="px-4 py-[7px] text-[11px] text-white/45">Grade {v.gradeLevel}</td>
                            <td className="px-4 py-[7px] text-[11px] text-white/45">Section {v.section}</td>
