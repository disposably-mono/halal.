...
export function AdminHelpGate() {
  ...
  return (
    <div className="fixed inset-0 z-70 flex items-center justify-center bg-[#070919]/85 px-5 backdrop-blur-md">
      <div role="dialog" aria-modal="true" aria-labelledby="admin-help-title" className="w-full max-w-sm overflow-hidden border border-gold/20 bg-[#151b36] shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div className="space-y-2">
            ...
            <SecretInput
              ...
              className="h-11 border border-white/10 bg-white/5 px-3 font-mono text-sm text-white outline-hidden placeholder:text-white/25 focus:border-gold/50 focus:ring-2 focus:ring-gold/15"
              placeholder="Enter officer key"
            />
          </div>
          {state?.error && <p id="help-key-error" role="alert" className="border border-red-400/20 bg-red-400/8 px-3 py-2 text-xs text-red-300">{state.error}</p>}
          <div className="flex gap-2.5">
            <Link href="/" className="flex h-10 flex-1 items-center justify-center border border-white/10 text-xs text-white/50 transition-colors hover:bg-white/4 hover:text-white/75">Back home</Link>
...
function UnlockButton({ isPending }: { isPending: boolean }) {
  return (
    <button type="submit" disabled={isPending} className="h-10 flex-1 bg-gold text-xs font-bold text-navy-deep transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
