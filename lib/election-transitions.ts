...
export async function applyScheduledTransitions(): Promise<TransitionSummary> {
  return withSpan("cron.transition_sweep", {}, async (span) => {
    const now = new Date();
    ...
    span.setAttribute("transition.opened_count", openedIds.length);
    span.setAttribute("transition.closed_count", closedIds.length);
    span.setAttribute("transition.missed_window_count", missedClosedIds.length);
