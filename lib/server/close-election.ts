...
/**
 * The certification + close transaction body, taking an already-open `tx` so
 * an admin-facing caller (see closeElectionNow in
 * elections/[id]/control/actions.ts) can run it inside the same transaction
 * as its own pre-check, while `closeElectionWithCertification` below keeps
 * opening its own transaction for callers that don't already have one (the
 * cron sweep in lib/election-transitions.ts).
 */
export async function closeElectionCore(
...
export async function closeElectionWithCertification(
  ...
  return prisma.$transaction((tx) =>
    closeElectionCore(tx, electionId, actor, allowedStatuses, auditActionOverride),
  );
}
